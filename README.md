...
- **Claude usage** (v3.1.7, opt-in) — a card pinned at the top showing your live session (5h) and weekly limits, including per-model buckets, exactly like the Claude settings Usage panel. Colours by severity, with reset countdowns. If you've turned on **Usage credits** (pay-as-you-go) in Claude settings, it also shows a credits row (spend vs. monthly limit); it stays hidden otherwise. Off by default; enable it from the invite card on the board or via `overlord.usage`. When on it reads your local Claude login and does one plain usage read a minute (0 tokens, not an AI call) — see Privacy below.
