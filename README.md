...
- **Session telemetry** (v3) — `state + elapsed · model · subagents` on the status line, `ctx usage · uptime` beneath it. Context is always the real token count (`ctx 138k`), never a percentage — a percent would need the session's true window, which varies by model (200k vs 1M) and isn't knowable from outside.
- **Launch pills** (v3) — up to 3 configurable buttons above the board. Each opens an editor-area terminal in its own folder and types its own command (e.g. `claude`, or your own alias). Configure via the ⚙ button (opens all Overlord settings); slots ship empty; optional auto-launch on VS Code start.
...
- **Claude usage** (v3.1.7, opt-in) — a card pinned at the top (or bottom, via `overlord.usagePosition`) showing your live session (5h) and weekly limits, including per-model buckets, exactly like the Claude settings Usage panel. Colours by severity, with reset countdowns. If you've turned on **Usage credits** (pay-as-you-go) in Claude settings, it also shows a credits row (spend vs. monthly limit); it stays hidden otherwise. Off by default; enable it from the invite card on the board or via `overlord.usage`. When on it reads your local Claude login and does one plain usage read a minute (0 tokens, not an AI call) — see Privacy below.
![The Overlord status bar: session counts, sound toggle, window toggle, new session](media/statusbar_vscode.png)

- **Status-bar quick toggles** (v3.1.21) — next to the session summary at the bottom right: a speaker button to mute/unmute the "needs you" sound, and a window button to switch between this window's sessions and every session on the machine. One click each; the icon shows the current state, and it follows the setting however you change it.
- **Several VS Code windows** (v3.1.25) — the board is machine-wide, so every window lists every session, and now every card says which window it lives in: `in FAI`, or `no terminal` when its tab was closed and the process kept running. A card belonging to another window offers `Go to FAI ↗`, which reveals the terminal there and brings that window forward. The status-bar tooltip breaks the count down the same way (`needs you: 1 here · 1 in FAI`), and the alert sound plays once per machine rather than once per open window. With a single window nothing changes. On macOS, raising one specific window needs Accessibility permission for VS Code; without it Overlord brings VS Code forward and you pick the window.
...
| `overlord.usage` | `false` | show your Claude usage limits pinned to the board (opt-in; reads your local login + one usage read/min, 0 tokens — see Privacy) |
| `overlord.usagePosition` | `top` | where the usage card sits: `top` (sticky header above the sessions) or `bottom` (pinned to the panel's bottom edge, sessions scroll above it) |
