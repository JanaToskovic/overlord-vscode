...
## 3.1.19 — 2026-08-03
- **Fix: the usage card now works on macOS.** On a Mac, Claude Code stores your login in the encrypted macOS **Keychain** — not in `~/.claude/.credentials.json`, which is where the card was looking (that file only exists on Windows/Linux). So every Mac user saw a permanent "No Claude login found — run any Claude Code session first", and neither logging in nor ↻ could ever fix it. The card now reads the Keychain first on macOS (item `Claude Code-credentials`, the same one Claude Code itself uses), with the file kept as a fallback everywhere — the file is also the documented workaround for SSH/container setups on Macs, so both arrangements work. `CLAUDE_CONFIG_DIR` is honored too, on every OS. Keychain reads are cached for 5 minutes; ↻ and a rejected token both force a fresh read, so a re-login is picked up immediately. Windows/Linux behavior is unchanged.
- **Honest "no login" wording on a Mac.** The old advice ("run any Claude Code session first") sent macOS users in circles. On a Mac it now says what was actually checked: the Keychain and `~/.claude`.
- **The card's context readout is now always the real amount ("ctx 138k"), never a percent.** The percent was computed against an assumed 200k window, but a session on a 1M-context model reports the same model name — so 138k used showed as "ctx 69%" (nearly full) on a session actually at ~14%. An absolute number can't lie about a window it doesn't claim to know; the tooltip's "138k/200k" denominator is gone for the same reason. Sessions genuinely near the limit still read exactly as before ("ctx 247k").

## 3.1.18 — 2026-07-20
- **Fix: a session no longer blinks "awaiting your reply" over the word "approved".** The go-ahead detector matched past-tense "approved … and I'll" (e.g. "the plan is approved and I'll start it") as if it were an approval request. That's a statement, not a question, so the card wrongly turned red. It now only matches the imperative forms actually aimed at you (confirm / approve / say go / sign off); real go-ahead asks still flag correctly.
- **The launch pills stay put when you scroll.** The pills row is now pinned together with the usage card at the top of the panel, instead of sliding up and disappearing underneath it.
- **New: ↗ on the usage card opens the real usage page.** A one-click link to claude.ai's usage settings, in the card header. Handy when the card itself can't fetch (rate-limited or login expired) and you want to check your actual numbers in the browser.

## 3.1.17 — 2026-07-17
- **Fix: the usage card never opens empty again.** Your last good numbers now survive reloads, restarts and extension updates: if a check fails (or a rate limit is in force) the card shows the previous numbers greyed out with a note, instead of a bare "Rate-limited…" line and nothing else. Restored numbers recompute their reset countdowns from the stored reset times, and snapshots older than a day are not restored.
- **Fix: a restart during a rate-limit window no longer probes straight back into it.** The server-declared wait (Retry-After) is remembered across restarts; on startup Overlord shows your last numbers greyed and schedules the next check for when the window lifts, rather than burning a guaranteed-429 call immediately. The ↻ button still forces an immediate attempt.
- **Honest error notes.** A network timeout, a server error and a real rate limit used to all render as "Rate-limited by the usage API". The note now says which one actually happened ("Can't reach the usage API" / "Usage API error (503)" / "Rate-limited by the usage API").

## 3.1.16 — 2026-07-17
- **Fix: the agent badge no longer counts agents that already finished.** A card could sit showing "2 agents" with nothing actually running. Backgrounded agents announce they're done in a notification keyed to their internal agent id, carrying none of the tool id the badge was matching on (backgrounded shell commands do carry it, which is why those always cleared correctly). So a backgrounded agent had no completion signal at all: it entered the pending list at launch and never left, and the count only ever went up. The launch receipt is now read for the agent's id, which is what links a launch to the notification that later reports it finished. An agent you stop early is also cleared now, via its TaskStop receipt, since a killed agent never sends a completion at all. A genuinely running agent still counts, exactly as before.

## 3.1.15 — 2026-07-17
- **Fix: a card you've already looked at no longer goes back to blinking red on its own.** Clicking a red "needs you" card greys it out to mark it seen, but that mark was tied to the transcript file's last-modified time — so *any* write to the file un-did it, even one that added no new message. An idle session's transcript kept getting touched with identical content (last message 15:18, file clock 23:11), which quietly expired the mark and sent the card back to the top of the board, blinking, with nothing new to show. "Seen" is now tied to the last actual message in the conversation, so it survives file touches and bookkeeping records, and still clears the moment the session genuinely says something new.
